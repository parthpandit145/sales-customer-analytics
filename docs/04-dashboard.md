...
- Sync those slicers across all three pages (View → Sync slicers), but **do not**
  sync visibility — each page shows the strip.
...
Use `map_location` (`"Sao Paulo, Brazil"`) rather than the two-letter code —
...
low → accent high. Turn row/column subtotals **off** — retention totals are
